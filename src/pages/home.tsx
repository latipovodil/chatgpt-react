...
export default function Home() {
  ...
  useEffect(() => {
    if (user) {
      const userData2 = JSON.parse(user);
      if (userData2?.operationType) {
  ...
  React.useEffect(() => {
    if (send.length) {
      const API_KEY = import.meta.env.VITE_APP_TITLE;
        ...
        <div className="main">
          ...
          <div className="main__right">
                  ...
                  ? send?.map((el: any) => (
                      <div className="main__message" key={el.id}>
                        <div className="main__message-user" id={el.id}>
                          <img
                            src={userData ? userData.user.photoURL : ""}
                            alt="avatar"
                          />{" "}
                          <p>{el.value}</p>
                        </div>
            ...
            {img ? (
              ...
              >
                Rasm chizishi uchun faqatgina ingliz tilida yozish kerak !!!
              </p>
            ) : (
              ""
            )}
